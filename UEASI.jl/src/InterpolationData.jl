...
# Fix for "Too many open files" error when generating all the plots
# https://github.com/JuliaPlots/Plots.jl/issues/1723
GR.inline("png")
    ...
    for i in 1:length(series.measurements)
        ...
        if ismissing(series.measurements[i])
            ...
            if interpcount > 0
                ...
                # We set it to 0.001 so the temporal uncertainty can be
                # distinguished from spatial uncertainty when drawing plots
                interpolateduncertainties[i] = 0.001
            end
        end
    end
...
# Replace missing values in an array with zeros
function _missingtozero(series)
  series[ismissing.(series)] .= 0
end

# Plot a cell's details
function plotcell(cell::Cell, folder::String)
    data::InterpolationCellData = _loadinterpolationdata(cell)

    if (!data.land)
        # Replace missings in uncertaintes with zeros
        _missingtozero(data.originalinputuncertainties)
        ...
        _missingtozero(spatialinterpolateduncertainties)
